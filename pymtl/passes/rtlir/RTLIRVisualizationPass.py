#=========================================================================
# RTLIRVisualizationPass.py
#=========================================================================
# Visualize RTLIR using Graphviz packeage. The output graph is in PDF
# format. 
...
class RTLIRVisualizationPass( BasePass ):
  def __call__( s, model ):
    visitor = RTLIRVisualizationVisitor()

    for blk in model.get_update_blocks():
      visitor.init( blk.__name__ )
      visitor.visit( model._pass_component_upblk_rtlir_gen.rtlir[ blk ] )
      visitor.dump()

class RTLIRVisualizationVisitor( RTLIRNodeVisitor ):
  def __init__( s ):
    s.output = 'unamed'
    s.output_dir = 'rtlir-viz'

  def init( s, name ):
    s.g = Digraph( 
      comment = 'RTLIR Visualization of ' + name,
